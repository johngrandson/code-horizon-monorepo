defmodule PetalPro.AccountsTest do
  ...
  describe "fixtures" do
    test "#admin_fixture/1" do
      admin = admin_fixture()

      assert(admin.role == :admin)
      assert(admin.is_suspended == false)
      assert(admin.is_deleted == false)
      assert(admin.is_onboarded == false)
      assert(admin.current_impersonator == nil)
    end

    test "#impersonator_fixture/1" do
      %{user: user, admin_user: admin_user} = impersonator_fixture()

      assert(user.role == :user)
      assert(user.is_suspended == false)
      assert(user.is_deleted == false)
      assert(user.is_onboarded == false)
      assert(user.current_impersonator == admin_user)
  ...
  describe "register_user/2" do
    ...
    test "registers a user from external_provider" do
      user_attrs = %{email: unique_user_email(), name: "John", avatar: "avatar.jpg"}
      {:ok, user} = Accounts.register_user(user_attrs, "external_provider")
      assert user.email == user_attrs.email
      assert user.name == user_attrs.name
      assert user.avatar == user_attrs.avatar
      assert is_binary(user.hashed_password)
      assert is_nil(user.confirmed_at)
      assert is_nil(user.password)
      ...
      assert PetalPro.Logs.exists?(
               ...
               action: "register",
               metadata: %{
                 registration_type: "external_provider"
               }
             )
    end

    test "registers a user from passwordless" do
      email = unique_user_email()
      {:ok, user} = Accounts.register_user(%{email: email}, "passwordless")
      assert user.email == email
      assert is_binary(user.hashed_password)
      assert is_nil(user.confirmed_at)
      assert is_nil(user.password)
      ...
      assert PetalPro.Logs.exists?(
               ...
               action: "register",
               metadata: %{
                 registration_type: "passwordless"
               }
             )
    end
  end
