defmodule PetalPro.Accounts.UserPin do
  @moduledoc false
  use PetalPro.Schema
  ...
  defp validation_status(%__MODULE__{} = user_pin, pin, allowed_attempts) do
    expired? = Timex.diff(DateTime.utc_now(), user_pin.inserted_at, :minutes) >= @pin_validity_in_minutes
  ...
  defp get_hashed_pin(pin),
    do: :crypto.mac(:hmac, :sha256, Application.get_env(:petal_pro, PetalProWeb.Endpoint)[:secret_key_base], pin)
