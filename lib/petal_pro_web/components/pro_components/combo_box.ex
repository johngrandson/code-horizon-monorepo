defmodule PetalProWeb.ComboBox do
  @moduledoc false
  ...
  attr(:remote_options_event_name, :string,
    default: nil,
    doc: "The event name to trigger when searching for remote options. That event must return a li"
  )
  ...
  def combo_box(%{field: %Phoenix.HTML.FormField{} = field} = assigns) do
    ...
    |> assign(:errors, Enum.map(errors, &PetalProWeb.CoreComponents.translate_error(&1)))
