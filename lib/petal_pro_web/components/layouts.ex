defmodule PetalProWeb.Layouts do
  ...
  alias PetalProWeb.LandingPageComponents
  ...
  def current_page_url(%{request_path: request_path}), do: PetalProWeb.Endpoint.url() <> request_path
  ...
  def twitter_site(_conn) do
    if PetalPro.config(:twitter_url) do
      "@" <> (:twitter_url |> PetalPro.config() |> String.split("/") |> List.last())
    else
      ""
    end
  end
  ...
  def public_page?(request_path) do
    ...
    Enum.find(PetalProWeb.Menus.public_menu_items(), &(URI.parse(&1.path).path == stripped_path))
  end
end
