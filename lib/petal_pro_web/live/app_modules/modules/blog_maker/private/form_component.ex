defmodule PetalProWeb.BlogMakerLive.FormComponent do
  @moduledoc false
    ...
    case Posts.update_post(post, post_params) do
      {:ok, post} ->
        ...
        {:noreply,
         ...
         |> push_navigate(to: ~p"/app/org/#{socket.assigns.org.slug}/blog-maker/#{post.id}")}
  ...
  def handle_event("close_modal", _, socket) do
    {:noreply, push_patch(socket, to: ~p"/app/org/#{socket.assigns.org.slug}/blog-maker/#{socket.assigns.post.id}")}
  end
  ...
  def handle_event("select_file", %{"image-target" => "cover"} = file_params, socket) do
    ...
    socket =
      socket
      |> assign(form: form)
      |> push_patch(to: ~p"/app/org/#{socket.assigns.org.slug}/blog-maker/#{socket.assigns.post.id}/show/edit")
