defmodule PetalProWeb.BlogMakerLive.PublishComponent do
  @moduledoc false
    ...
    case Posts.unpublish_post(socket.assigns.post) do
      {:ok, post} ->
        ...
        {:noreply,
         ...
         |> push_patch(to: ~p"/blog-maker/#{post.id}")}
  ...
  defp save_post(socket, :publish, post_params) do
    case Posts.publish_post(socket.assigns.post.org.id, socket.assigns.post, post_params) do
      {:ok, post} ->
        notify_parent({:saved, post})
        ...
        {:noreply,
         ...
         |> push_navigate(to: ~p"/blog-maker/#{post.org.slug}/#{post.slug}")}
