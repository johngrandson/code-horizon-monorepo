defmodule PetalProWeb.UserAiChatLive do
  @moduledoc """
  The user AI Chat live view.

  **In case you need some more rules for the Chatbot to adhere to, just expand on the Markdown in the `PetalProWeb.UserAiChatLive.OpenAI.default_messages/0` function.**
  """
  use PetalProWeb, :live_view

  alias PetalProWeb.LangChainHelpers
  alias PetalProWeb.UserAiChatLive.LangChain
  alias PetalProWeb.UserAiChatLive.Message
  ...
  def mount(_params, _session, socket) do
    {:ok,
     socket
     |> assign(
       page_title: gettext("User AI Chat"),
       loading: false,
       ...
       messages: [
         %{
           role: :assistant,
           content: """
           Hey there! I'm the User AI Chat built with LangChain. I'm here to help you with any questions you have.

           For example: you can ask me about your user profile data or the pages on this website.

           _Tip: You can also press and hold the microphone button to speak with me_
           """
         }
       ],
       assistant: LangChain.assistant(self(), socket.assigns.current_user)
     )
            ...
            [
              %{
                ...
                *Received rate limit from OpenAI, we're waiting for the exact time (#{timeout}#{unit}) to continue.*
                ...
                To find out more about their rate limiting policy, please visit their <a href="https://platform.openai.com/accounts/rate-limits" target="_blank" class="font-bold underline">documentation</a>.
                """
              }
            ]
